...
data class ItemWiseSalesUi(
    val isLoading: Boolean = false,
    val data: ItemWiseSalesResponse? = null,
...
class ItemWiseSalesViewModel (private val repo: DailyCollectionRepo): ViewModel(){
    private val _uiState = MutableStateFlow(ItemWiseSalesUi())
    ...
    fun fetch(from: String, to: String) {
        ...
        viewModelScope.launch {
            val result = repo.getItemWiseSales(from, to)
            ...
            result.onFailure { err ->
                _uiState.value = _uiState.value.copy(
                    isLoading = false,
                    isError = true,
                    snackbarMessage = (err as AppError).userFriendlyMessage
                )
            }
