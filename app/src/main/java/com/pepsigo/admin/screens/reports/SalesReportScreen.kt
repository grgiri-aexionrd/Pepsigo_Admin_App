...
//                    .padding(16.dp)
                ...
                Column(
                    ...
                    DockedDatePicker(
                        ...
                        error = salesReport.fromDateError,
                    ...
                    DockedDatePicker(
                        label = stringResource(R.string.select_date),
                        error = salesReport.toDateError,
                        modifier = Modifier.fillMaxWidth(),
                        onDateSelected = { date -> viewModel.setToDate(date) }
                    )
                    Text(
                        text = stringResource(R.string.customer_dropdown),
                        style = MaterialTheme.typography.bodyLarge,

                        )
                    Log.d(
                        "SalesRegisterScreen",
                        "Rendering DropDown with selected: ${salesReport.selected}, options: ${salesReport.dropDown}"
                    )

                    DropDown(
                        dropDown = salesReport.dropDown,
                        error = salesReport.dropDownError,
                    ...
                    Button(
                        onClick = {
                            Log.d(
                                "SalesRegisterScreen",
                                "Apply Filter clicked with fromDate: ${salesReport.fromDate}, toDate: ${salesReport.toDate}, selected customer: ${salesReport.selected}"
                            )
                            // Trigger filter action in ViewModel
                            viewModel.fetchSalesRegister(
                                dateFrom = salesReport.fromDate,
                                dateTo = salesReport.toDate,
                                customerId = salesReport.selected?.id
                            )
                        ...
                        enabled = !salesReport.isLoading
                    ) {
                        if (salesReport.isLoading) {
